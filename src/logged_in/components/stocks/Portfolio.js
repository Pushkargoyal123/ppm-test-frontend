    ...
    function () {
      ...
      const fetchPortfolioHistory = async () => {
        ...
        if (eventInfo) {
          ...
          resultportfolio = await postData(
            "dreamNifty/portfolio/fetchportfoliohistory",
            body
          );
        } else {
          resultportfolio = await getData(
            "stock/fetchportfoliohistory/" +
              userData.id +
              "?ppmGroupId=" +
              groupId.group
          );
        ...
        if (resultportfolio.success) {
          ...
          resultportfolio.data.forEach(function (item, index) {
            item.averageBuyingPrice =
              item.totalBuyingPrice / item.totalBuyStock;
            item.totalBuyingPrice =
              item.totalBuyingPrice - item.totalSellingPrice;
      ...
      const fetchVirtualAmount = async () => {
        ...
        if (eventInfo) {
          ...
          result = await postData(
            "dreamNifty/portfolio/findvirtualamountyuserid",
            body
          );
        } else {
          result = await getData(
            "user/findvirtualamountyuserid?ppmGroupId=" + groupId.group
          );
        }
        if (result.success)
  ...
  const fetchActivePlan = async () => {
    if (groupId.group !== "") {
      const data = await getData(
        "plans/hasActivePlan?ppmGroupId=" + groupId.group
      );
            ...
            columns={[
              ...
              },
              {
                ...
                render: (rowData) => (
                  <ToolTip
                    title={
                      "See " + rowData.companyName + " Transaction History"
                    }
                    component={() => (
                      <span>
                        <Button
              ...
              },
              {
                ...
                render: (rowData) =>
                  "₹" + rowData.averageBuyingPrice.toFixed(2),
              },
              {
                    ...
                    <TableRow>
                      <TableCell className="tablecell" colSpan={8}>
                        ...
                        <div style={{ margin: 10 }}>
                          ...
                          <span style={{ color: "red" }}>₹{count * 10} </span>
                        </div>
                        <div style={{ margin: 10 }}>
                          Amount left in your bucket for buying stocks :{" "}
                          <span style={{ color: "red" }}>₹{virtualAmount}</span>
                        </div>
                        <div style={{ margin: 10 }}>
                          Net Amount :{" "}
                          <span style={{ color: "red" }}>
                            ₹
                            {(
                              parseFloat(totalBuyPrice) +
                              parseFloat(virtualAmount) +
                              totalPL
                            ).toFixed(2)}
                          </span>
                        </div>
                        ...
                        >
                          {userData.userName} is in{" "}
                          {totalPL > 0 ? "Profit" : "Loss"} of ₹
                          {totalPL.toFixed(2)}
                        </div>
                      </TableCell>
                    </TableRow>
