            ...
            <MaterialTable
              ...
              data={
                stockEvaluation.map(function (item, index) {
                  return { CompanyCode: item.companyCode, CompanyName: item.companyName, CurrentPrice: "₹" + item.currentPrice, HighPrice: "₹" + item.highPrice, LowPrice: "₹" + item.lowPrice, Variance: "₹" + item.variance, Signal: item.stockSignal, isPresent: item.isPresent }
                })
              }
              localization={{
                body: message === 1 ?
              ...
              actions={[
                rowData => ({
                  icon: () => <AddIcon style={{ color: rowData.isPresent ? "#FFC312" : "green", fontWeight: "800", fontSize: "1.7rem" }} />,
                  tooltip: 'Add Stock',
